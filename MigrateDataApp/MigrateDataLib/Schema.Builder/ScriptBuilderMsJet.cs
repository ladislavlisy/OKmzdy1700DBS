...
{
    public class ScriptBuilderMsJet : ScriptBuilderBase
    {
        public ScriptBuilderMsJet(DbsDataConfig config) : base(config)
        {
        }
        protected override string DbConvertDataType(TableFieldInfo fieldInfo)
        {
            string strFieldType = DBPlatform.MsJetConvertDataType(fieldInfo.ColumnType, fieldInfo.ColumnSize);
